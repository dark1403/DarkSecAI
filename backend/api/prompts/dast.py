...
# Prompt variations for different scan types
recon_variations = [
...
def create_recon_dast_analysis_prompt(url: str, iteration: int) -> str:
    """
    Creates the prompt for DAST (Recon Scan). Focuses on public information.
    
    Args:
        url: The URL to analyze.
        iteration: The current recursive iteration, used to select a prompt variation.
    ...
    focus = recon_variations[iteration % len(recon_variations)].replace('{hostname}', hostname)
...
def create_active_dast_analysis_prompt(url: str, iteration: int) -> str:
    """
    ...
    focus = active_variations[iteration % len(active_variations)].replace('{hostname}', hostname)
...
def create_greybox_dast_analysis_prompt(url: str, iteration: int) -> str:
    """
...
def create_dast_analysis_prompt(url: str, scan_type: str, iteration: int) -> str:
    """
    Factory function to create the appropriate DAST analysis prompt based on scan type.
    
    Args:
        url: The URL to analyze.
        scan_type: The type of scan to perform ('recon', 'active', or 'greybox').
        iteration: The current recursive iteration, used to select a prompt variation.
    ...
    """
    if scan_type == 'recon':
        return create_recon_dast_analysis_prompt(url, iteration)
    elif scan_type == 'active':
        return create_active_dast_analysis_prompt(url, iteration)
    elif scan_type == 'greybox':
        return create_greybox_dast_analysis_prompt(url, iteration)
    else:
